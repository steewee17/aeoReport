(function () {
  ...
  function injectFont() {
    ...
    l.rel = 'stylesheet'; l.href = FONT_URL; l.setAttribute('data-vg-font', '1');
  ...
  function injectStyles() {
    ...
    s.id = 'vg-styles';
    s.textContent = [
      '#vg-root *,#vg-root *::before,#vg-root *::after{box-sizing:border-box;margin:0;padding:0;}',

      // Widget root
      '#vg-root{position:fixed;bottom:20px;right:20px;z-index:2147483647;',
      'display:flex;flex-direction:column;align-items:flex-end;font-family:"Alegreya Sans",sans-serif;}',

      // Chat window
      '#vg-chat{width:380px;height:590px;background:#fff;border-radius:18px;overflow:hidden;',
      'box-shadow:0 12px 40px rgba(0,0,0,0.22);display:flex;flex-direction:column;',
      'transform-origin:bottom right;transform:scale(0.88) translateY(12px);opacity:0;pointer-events:none;',
      'transition:transform 0.24s cubic-bezier(.34,1.3,.64,1),opacity 0.2s ease;margin-bottom:12px;}',
      '#vg-chat.vg-open{transform:scale(1) translateY(0);opacity:1;pointer-events:all;}',

      // Header
      '.vg-header{background:' + C.p600 + ';padding:14px 16px 13px 16px;',
      'display:flex;align-items:center;gap:10px;flex-shrink:0;}',
      '.vg-header-text{flex:1;min-width:0;}',
      '.vg-header-title{font-size:15px;font-weight:700;color:#fff;line-height:1.25;letter-spacing:0.01em;}',
      '.vg-header-sub{font-size:11.5px;color:rgba(255,255,255,0.72);margin-top:1px;',
      'white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}',
      '.vg-btn-close{width:28px;height:28px;background:rgba(255,255,255,0.15);border:none;border-radius:50%;',
      ...
      // Messages area
      '#vg-messages{flex:1;overflow-y:auto;padding:18px 16px 10px 16px;',
      'display:flex;flex-direction:column;gap:12px;background:#faf8f6;}',
      ...
      // Bubble bot
      '.vg-bubble-bot{background:#fff;color:#1a1a1a;border:1px solid ' + C.p100 + ';',
      'border-radius:16px 16px 16px 4px;padding:11px 15px;font-size:14px;',
      'line-height:1.6;word-break:break-word;max-width:272px;}',

      // User row + bubble
      '.vg-row-user{display:flex;justify-content:flex-end;}',
      '.vg-bubble-user{background:' + C.primary + ';color:#fff;',
      'border-radius:16px 16px 4px 16px;padding:11px 15px;font-size:14px;',
      'line-height:1.6;word-break:break-word;max-width:272px;}',

      // System message
      '.vg-system{align-self:center;background:' + C.p50 + ';color:' + C.p400 + ';',
      'font-size:11px;padding:4px 12px;border-radius:20px;}',

      // CTA button
      '.vg-cta{display:inline-block;margin-top:9px;background:' + C.primary + ';color:#fff;',
      'text-decoration:none;padding:8px 18px;border-radius:22px;font-size:13px;',
      'font-family:"Alegreya Sans",sans-serif;font-weight:500;transition:background 0.15s;}',
      '.vg-cta:hover{background:' + C.p600 + ';}',

      // Typing dots
      '.vg-dots{display:flex;gap:4px;padding:3px 2px;}',
      '.vg-dots span{width:7px;height:7px;background:' + C.p300 + ';border-radius:50%;',
      'animation:vgBounce 1.2s infinite;}',
      '.vg-dots span:nth-child(2){animation-delay:0.2s;}',
      '.vg-dots span:nth-child(3){animation-delay:0.4s;}',
      '@keyframes vgBounce{0%,60%,100%{transform:translateY(0);}30%{transform:translateY(-5px);}}',

      // ── INPUT AREA ──
      // Outer wrapper: gives breathing room from widget edges
      '.vg-input-area{background:#fff;padding:10px 14px 11px 14px;',
      'display:flex;align-items:center;gap:8px;',
      'flex-shrink:0;border-top:1px solid ' + C.p100 + ';}',

      // Inner pill: visual container for icon + input field
      '.vg-input-pill{flex:1;display:flex;align-items:center;gap:8px;',
      'background:' + C.p50 + ';border:1px solid ' + C.p100 + ';border-radius:24px;',
      'padding:0 14px;height:42px;min-width:0;}',
      ...
      // Text input
      '#vg-input{flex:1;border:none;outline:none;background:transparent;',
      'font-size:14px;font-family:"Alegreya Sans",sans-serif;color:#1a1a1a;min-width:0;}',
      '#vg-input::placeholder{color:' + C.p300 + ';}',

      // Reset button (inside pill, right of input)
      '.vg-btn-reset{width:26px;height:26px;border:none;background:none;cursor:pointer;',
      'color:' + C.p300 + ';display:flex;align-items:center;justify-content:center;',
      ...
      // Send button — standalone outside the pill
      '.vg-btn-send{width:42px;height:42px;background:' + C.primary + ';border:none;border-radius:50%;',
      'cursor:pointer;display:flex;align-items:center;justify-content:center;',
      'flex-shrink:0;transition:background 0.15s;}',
      '.vg-btn-send:hover{background:' + C.p600 + ';}',
      '.vg-btn-send:disabled{background:' + C.p200 + ';cursor:not-allowed;}',
      '.vg-btn-send svg{width:15px;height:15px;margin-left:2px;}',

      // Footer
      '.vg-footer{background:#fff;padding:6px 14px 9px;text-align:center;',
      ...
      // Pill trigger button
      '#vg-pill{background:' + C.primary + ';color:#fff;font-family:"Alegreya Sans",sans-serif;',
      'font-size:15px;font-weight:500;border:none;border-radius:24px;padding:11px 22px;',
      'cursor:pointer;box-shadow:0 4px 16px rgba(174,67,65,0.35);transition:background 0.15s,transform 0.1s;}',
      '#vg-pill:hover{background:' + C.p600 + ';transform:scale(1.03);}',

      // Mobile fullscreen
      '@media(max-width:480px){',
      '#vg-chat{width:100vw;height:100dvh;border-radius:0;position:fixed;top:0;left:0;margin-bottom:0;}',
    ...
    div.innerHTML =
      '<div id="vg-chat">' +
        '<div class="vg-header">' +
          ...
          '</div>' +
          '<button class="vg-btn-close" id="vg-close" aria-label="Schlie\u00dfen">\u00d7</button>' +
        '</div>' +
        '<div id="vg-messages">' +
        ...
        '<div class="vg-input-area">' +
          '<div class="vg-input-pill">' +
            ...
            '</button>' +
          '</div>' +
          '<button class="vg-btn-send" id="vg-send" aria-label="Senden">' +
